...
class Pipe:
    ...
    def responses(
        ...
        if command == "credits":
            ...
            credits = (
                ...
                "- **Timothy J Baek** for creating the beautiful monster that is [Open WebUI](http://github.com/open-webui/open-webui), the original Snake game implementation, and his final push over my roadblock to getting this to actually work.\n"
            ...
            # Use the format_credits method to yield the centered lines one by one
            return self.format_credits(credits)
        else:
            list_of_responses = [
            ...
            if existing_html:
                ...
                html_content = html_content.replace(
                    ...
                    f'Module.FS.createPreloadedFile("", "doom1.wad", "{self.get_file_url(wad_id)}", true, true);',
                )
                html_content = html_content.replace(
                    'Module.FS.createPreloadedFile("", "default.cfg", "default.cfg", true, true);',
                    f'Module.FS.createPreloadedFile("", "default.cfg", "{self.get_file_url(cfg_id)}", true, true);',
                )
                html_content = html_content.replace(
    ...
    def format_credits(self, credits: str) -> Generator:
        lines = credits.split("\n")
        for line in lines:
